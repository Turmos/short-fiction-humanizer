...
- `mini`：把 `dist/short-fiction-humanizer-mini.md` 放进一次性 prompt 或 `SOUL.md` 的风格段；它没有 skill 元数据，不应冒充可自动发现的 skill。
...
```bash
mkdir -p workspace/skills/short-fiction-humanizer
cp SKILL.md workspace/skills/short-fiction-humanizer/
cp -r references workspace/skills/short-fiction-humanizer/
```
...
```markdown
## 说人话
所有对外文本（消息、文档、摘要、公开写作）遵循 `skills/short-fiction-humanizer/SKILL.md` 的规则。
...
```bash
git add workspace/skills/short-fiction-humanizer
git commit -m "feat: add short-fiction-humanizer skill"
git push
```
