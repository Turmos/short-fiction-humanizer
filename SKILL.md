---
name: short-fiction-humanizer
description: Humanize AI-sounding Chinese short fiction and polish awkward narrative or dialogue while preserving plot, voice, pacing, logic, character motives, terminology, and factual relationships. Best for 8k–30k Chinese stories, outlines, rewrites, and editorial review.
---
...
- 需要改写或审查 8k–3w 字中文短篇小说、短篇大纲或续写片段
- 需要先判断文本该轻改、中改还是重改
...
1. 判场景：`chat / status / docs / public-writing / short-fiction`
...
5. 判 scope：`structural / bounded / in-place`，判断这次能删到什么程度——自由删并重排、只把整句空话进删除清单、还是一句都不删。`short-fiction` 默认 `bounded`；只有用户明确要求整篇重构/删减才用 `structural`。
6. 先执行本文件里的最小规则；只要环境里能读 `references/`，默认继续按问题类型补看 [Protected Spans](./references/protected-spans.md)、[Positive Style Contract](./references/positive-style.md)、[微操作手册](./references/operation-manual.md)、[结构反模式](./references/structures.md) 和相关短语表。判定为 `short-fiction` 时，必须补看 [中文短篇小说去 AI 规范](./references/short-fiction.md)；如果目标是“改完能直接发”，或文本明显属于 README、release note、论坛帖、issue 回复、API reference、FAQ，再补看 [Scene Packs](./references/scene-packs.md) 和 [改写示例](./references/examples.md)
...
- 中文小说正文、短篇大纲、章节续写或“照这个设定重写”的片段
- 目标是保留爽点、反转和人物关系，同时降低 AI 腔、模板腔或翻译腔

默认档位：`standard`；若用户明确要求整篇重构或原稿出现密集结构病灶，可升为 `aggressive`。

短篇模式有自己的优先级：先保住故事事实、人物选择和因果，再处理语言；结构问题不能靠换几个同义词掩盖。执行时：

1. 建立“剧情账本”：人物当前处境、目标、选择、代价、已埋线索、设定规则和反转位置；禁止改写时凭空增加能力、资源、关系或情节结果。
2. 先做结构诊断，再做句子润色：检查前三句钩子、单线主线、冲突结果、主角主动性、核心反转（通常在全文 60%–85%）、揭示长度和结尾落点。
3. 把“像人”落实为可观察的动作、对白、物件和环境反馈。删掉替代场面的漂亮总结，不用新金句把空缺填回去。
4. 任何系统/游戏机制都要经过“规则 → 操作 → 环境或敌人如何受影响 → 限制或代价 → 结果”才能进入正文；不要堆系统弹窗，也不要在设定未授权时把不成立的因果写成拟人化召唤。
5. 默认保留少量自然的毛糙、停顿和句式变化；短篇不是每句话都抛光成同一长度。发现说话不完整、断词、断标点或人不会说的自造句，优先修清楚，再决定是否压缩。
6. **强制禁用“条件触发 + 资源对照 + 先缺/先出结果”的口号骨架**。例如“那地方一旦被打穿，城里不缺枪，先缺水”。这类句子把三个判断压成整齐的答案腔，生成或改写短篇正文时不得使用；同义变体（“一打穿就……，枪有的是，水先断”“如果……，不缺 A，先缺 B”）同样禁用。改成两个有动作或后果的句子，或写出具体的供给、损耗和人物选择。只有用户明确提供的引用原文或非小说技术说明可保留。
7. **强制过滤死板机械的“职能宣言 + 物件拟人 + 伪格言对白”**。例如“我负责让该关的门关上”“卡车没有变新，只是能跑了”“账比人还难伺候”“招牌不卖货，货架上的东西才卖钱”。这类句子看似有性格，实际是抽象职责、万能对比或物件替人物总结，连续出现时尤其像模型写出的角色金句。生成或改写小说正文时不得保留这种句式及同义变体；改成当下能看见的动作、具体故障、数字变化、利益冲突或人物没说满的口语。只有用户明确要求保留的引用、口号或特定人物的固定台词可以原样保留。
8. 处理成年人物的暧昧、擦边情趣场景时，直接进入场面，不先写创作说明或安全声明。情趣来自衣物状态、空间距离、触碰和回应的连续变化，不来自身体部位清单或“魅惑、撩人、勾魂”等空泛形容。每个服装细节都要和动作有关（湿衣服贴住、肩带滑落、领口被拉近、外套被拿走），每句对白都要有当下目的、试探或回避；禁止把对白写成轮流抛出的挑逗金句。保持非露骨：可写衣物、肩颈、锁骨、腰线、腿部、亲吻和双方自愿的暧昧触碰，不写生殖器、裸露性行为或具体性行为过程。人物年龄不明时，不把场景性化。
9. **正式剧情推进时保护读者注意力**。不要在冲突、判断或对白正往前走时，突然插入与当前选择无关的动作、环境或感官描写，把读者注意力带开。动作和环境只能承接正在发生的事，并至少改变选择、关系、风险、资源或信息中的一项；只是为了“制造画面”“像真人”或填节奏的细节删掉，必要时压成一句。对用户称作“压算认催”的处理保持克制：只有词义、人物意图和剧情后果都明确时才使用，不能把它当作节奏按钮、转场借口或自造术语。
10. **禁止不完整动宾句和悬空动词**。短篇正文中，凡是“拖、耗、欠、逼、管、接、靠、等、陪、救”等需要说明对象、方式、范围或后果的动词，不要只写成“我不拖你”“我不欠你”“我不逼你”“这事我不管”“这单我接”。有一个代词不等于信息完整：仍要让读者知道拖的是什么、怎么拖、欠了什么、逼到哪一步、管哪件事、接下什么，以及会造成什么结果。优先补出原文已有的对象或后果；补不清就改成具体动作或可见结果，不得凭空新增金额、期限、关系或责任。“靠！”等独立感叹词，以及“靠着墙”“靠这笔钱过冬”等对象明确的用法不误判。上下文已经交代清楚时可保留自然省略，但不能为了像口语而省掉关键成分。
11. **硬伤优先于小修辞**。发现以下问题时，先停下去 AI 味润色，修正事实、动作链和场面功能：对白意思不完整且不生活（如“我不拖你”却没有具体后果）、物件与菜品或场景尺度/质感不匹配（如大锅菜写成“小铝锅”）、同一句或同一事件疑似剪辑残留、前后行动互相打脸（已经叫过跑腿且无人接，后文又说“可以给他叫跑腿”）、搭配像会议或说明书（如“已经订好了”用于一顿临时聚餐）、无关的可爱或装饰细节打散正在收束的悲伤/紧张情绪，以及“手机提示某人发来一条语音”这类把现场写成通知栏的说明句。硬伤没有修完前，不得用新增感官、金句或生活素材转移注意力。

短篇模式的完整规则、反模式样例、句级别扭表达审校和交付检查见 [中文短篇小说去 AI 规范](./references/short-fiction.md)。静态扫词只能提供线索，不能替代上下文判断；需要快速复查时可运行 `scripts/audit_short_fiction.py`。
...
- `faq`：出现 FAQ / 常见问题 / 排障问答等发布结构时，先给结论或动作，再给条件、步骤和限制；不扩大问题范围、否定、期限与支持承诺，也不把采访或讨论问答套成 FAQ
...
## Short-fiction anti-template additions

短篇正文还要强制过滤以下问题：

- **前言不搭后语、无效对话和废话文学口号**：如“这不是荒岛求生，这是修理工加班”“别修了，修好了也没有电”。这类句子硬拼不在同一关系链上的概念，或否定动作后只补无效结果。回到具体对象、动作、阻力和反应；一句对白若不改变信息、关系、选择或情绪，就删掉或并入动作。故意说废话时，必须看得出敷衍、嘴硬、误会或打岔的功能。
- **动作接龙**：禁止“甲抬头，乙皱眉，丙后退，所有人沉默”的逐人同类动作清单。让人物同时做不同的事，互相打断，或因利益和位置不同产生不一致的反应，并留下具体阻力或代价。
- **感官钉子**：可加入少量不推动情节、但固定疲劳、温度、气味、疼痛或空间压力的细节，例如手指沾油、湿衣服贴背、嘴里有铁锈味、膝盖发酸。每场一两处，禁止成串感官词和抽象气氛句。
- **口语和碎片信息**：对白允许停顿、语气词、重复、改口和抢话，但要保留人物目的和身份。弹幕、评论、系统提示只保留影响判断或体现现场的内容，删掉“震惊、反转、打脸、太燃了”等盖章词和替作者讲解剧情的整句；情绪通过迟疑、嘴硬、回避、失手和关系变化推进，不要每隔几句宣布一次结果。若用户或作品格式明确要求系统提示一条条排队显示，系统行属于 protected span，不要一锅端改成旁白；只删同一事实的重复播报，提示后的叙述改写成新的动作、反馈或代价。真实的直播间损人碎片（如“【下一位，别浪费直播间流量。】”）也不要仅因带括号就删除。
- **文笔和动作链**：去 AI 不是把句子弄糙、弄短，也不是硬塞修理工黑话。优先使用普通、准确、读者一眼能懂的词；动作按发生顺序写清楚，让声音、视线、手上动作和对白互相接上。连续动作可以合在一个自然长句里，例如“我正蹲在船坞边拧螺丝，突然听见发动机的声音，只见三艘快艇正在飞速驶来”，不要改成“螺丝只咬半圈”一类自造术语。一个事实只呈现一次：旧句、新句、系统行和旁白不能并排复述同一结果。
- **推进节奏**：参考短段落、快对白和密集的小兑现，但不要复制“动作一段、反应一段、弹幕一段”的固定循环。每段至少有一个新动作、判断、资源变化或关系摩擦；章末留下下一步问题。人物的爽点来自对手的选择失误、资源损失和规则兑现，不靠连续羞辱、擦边或“全网震惊”盖章。
