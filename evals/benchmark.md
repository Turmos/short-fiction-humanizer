...
>
> 先运行 `short-fiction-humanizer config path` 查看实际读取路径。如果文件不存在，复制 `config.example.toml` 到该路径；如果文件已存在，检查环境变量 `SHUORENHUA_CONFIG` 是否指向旧目录。Windows 便携版不读取用户主目录下的配置文件。

**理由**：问题是用户会实际检索的问法，答案直接给步骤、分支和限制。`config file not found`、`short-fiction-humanizer config path`、`config.example.toml`、`SHUORENHUA_CONFIG`、`Windows` 都必须逐字保留，最后一句的“不读取”不能软化。整体应 `no-op`；不得补客服寒暄、改命令、删分支或虚构默认路径。
