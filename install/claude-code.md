...
```text
/plugin marketplace add Turmos/short-fiction-humanizer
/plugin install short-fiction-humanizer@short-fiction-humanizer
```

在 Claude Code 对话里执行这两条命令即可，skill 自动发现和触发。升级用 `/plugin` 面板或 `claude plugin update short-fiction-humanizer`。

plugin 自带全量文件（`SKILL.md` + `references/`），安装时不用选择档位。已经用下面方式 2-4 手动装过的，先移除旧安装再装 plugin，避免同一个 skill 重复触发。

## mini / lite / full 怎么选（手动使用时）

- `mini`：只把 `dist/short-fiction-humanizer-mini.md` 贴进单次会话。适合上下文很紧或较弱模型；它不参与 skill 自动发现。
...
```bash
mkdir -p .claude/skills/short-fiction-humanizer
cp SKILL.md .claude/skills/short-fiction-humanizer/
cp -r references .claude/skills/short-fiction-humanizer/
```
...
cp -r short-fiction-humanizer ~/.claude/skills/short-fiction-humanizer
```
...
```bash
git clone https://github.com/Turmos/short-fiction-humanizer.git
ln -s "$PWD/short-fiction-humanizer" ~/.claude/skills/short-fiction-humanizer
```
...
```markdown
## 写作风格
当任务涉及“去 AI 味”“说人话”“自然一点”“别像模板”这类改写时，遵循 `.claude/skills/short-fiction-humanizer/SKILL.md`。
