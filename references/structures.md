...
作者刻意用对比结构承载全文论点时，属于个别保留条件，不作为通用豁免口。
...
**模式**：先用“一旦/如果/真要”触发灾难，再用“不缺 A，先缺 B”或“有的是 A，先断 B”压缩资源优先级。
...
这类三拍结构在短篇正文中强制禁用，原因是它把条件、比较和结论排成预制答案，缺少现场动作。不要只替换“先缺”的同义词；拆成供给变化、人物反应或实际选择：

```
✅ 那地方真被打穿，供水线会先断。枪还能从仓库里找，城里的人得先想办法弄到水。
```

引用原文和必要的技术说明不受这条小说表达禁令影响。
...
**模式**：人物用抽象职责开口，物件被拟人化，再用“没有 X，只是 Y”“X 比人还 Y”“X 不做 A，Y 才有用”等对比句替现场下结论。
...
短篇小说正文中强制过滤这类死板机械、自造词感和伪金句对白，尤其是连续两句以上互相接答案的情况。`我信 X`、`X 也是人写的`、`那就把每一笔都摊开写`、`你是只盯着 A，还是也肯看 B`、`扣就扣，别把 X 做成好看的假数`这类句子也一样处理：它们会把对话写成概念接力。问题不在某个词，而在人物没有说眼前的事，却替作者概括职责、主题或常识。改写时落回可见动作和具体损失：哪一扇门、哪一项账、哪一处故障、谁要为差错买单。不要用另一组整齐的格言替换。
...
**模式**：动词看似带了宾语，实际没有交代对象、方式、范围或后果，人物像在发布抽象承诺。
...
**检测**：重点看上述动词是否出现在短对白、否定承诺或句末断句中；不能只按词面判死。`靠！` 等感叹词和 `靠着墙 / 靠这笔钱过冬` 等对象明确的字面用法放行。

**默认动作**：优先补原文已有的对象、做法、范围或结果；补不清就改写为具体动作或可见后果。不得凭空添加金额、期限、关系、责任或情节结果，也不得用同义词轮换掩盖缺项。

## 22.2 硬伤优先于小修辞

**模式**：文本已经有一些生活细节，但基础事实、动作链或情绪落点仍然出错。此时继续加细节只会把读者注意力带偏。

常见硬伤包括：

- 对白不完整且不像人物会说的话：`我不拖你`没有具体后果，不能靠加一个语气词修好；
- 物件与场景尺度/质感错配：大份分食的菜写成“小铝锅”；
- 同一节重复相同时间句或动作句，像剪辑残留；
- 已经叫过跑腿且对方没接，后文又说“我可以给他叫跑腿”，行动链互相冲突；
- `田螺鸭脚煲已经订好了`等会议式搭配替代了正在发生的生活动作；
- 悲伤或紧张收束段突然插入无关的可爱细节，打散情绪；
- `手机就提示某人发来一条语音`等通知栏式叙述，把人物现场切成设备播报。

**默认动作**：先做事实和场面修复，再做去 AI 润色。重复句保留一次；行动冲突改成放弃、换方案或承担失败后果；物件和菜品改回可见的端、盛、冒泡、分食等动作；消息保留但压掉设备拟人。不得用新金句、感官堆叠或“更生活”的装饰掩盖硬伤。

**保留条件**：有意复沓改变了语气、视角或信息；错配由设定或喜剧功能支持；通知句本身属于受保护的系统/消息列表格式。
