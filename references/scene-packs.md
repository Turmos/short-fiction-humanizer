...
- 问题范围准确；答案先给结论、判断或动作，再给必要条件、步骤、限制和下一步。
...
> 非常理解导出日志失败给你带来的困扰。关于导出日志失败怎么办，我们准备了简单方案：先运行 `short-fiction-humanizer logs export`。如果命令返回退出码 `2`，检查输出路径是否可写。

After:

> 先运行 `short-fiction-humanizer logs export`。如果命令返回退出码 `2`，检查输出路径是否可写。
