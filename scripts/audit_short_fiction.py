...
PATTERNS: list[tuple[str, str, str]] = [
    ...
    ("不完整动宾句/悬空动词", r"(?:我|你|他|她)(?:就|也|先|再|都|还|可)?(?:不|没|别|要不要)(?:拖|耗|欠|逼|管|接|等|陪|救)(?:我|你|他|她|它|这事|这单|这活)?[。！？!?]?$|(?:这事|这单|这活|这件事)(?:我|你|他|她)(?:就|也|先|再|都|还)?(?:不|没|别)?(?:管|接|拖|耗|欠|逼|等|陪|救)[。！？!?]?$", "检查是否缺少明确对象、方式、范围或后果；补原文已有信息，补不清就改成具体动作或结果。‘靠！’等感叹词和‘靠着墙/靠这笔钱’等对象明确用法放行"),
    ("通知栏式消息叙述", r"(?:手机|屏幕|页面)(?:就|上)?(?:提示|显示|弹出)[^。！？]{0,24}(?:发来|收到|一条(?:语音|消息|短信))", "保留消息事实，检查是否能和人物正在做的事相连；删掉无必要的设备拟人和说明书口吻"),
    ("会议式生活搭配", r"(?:饭|菜|火锅|鸭脚煲|田螺鸭脚煲)[^。！？]{0,12}(?:已经|早就)?(?:订好了|安排好了|预订完成)", "检查是否把临时聚餐写成会议安排；改成占桌、发照片、菜已下锅等正在发生的动作"),
    ("疑似器物质感/尺度错配", r"田螺鸭脚煲[^。！？]{0,16}小铝锅|小铝锅[^。！？]{0,16}田螺鸭脚煲", "核对菜品份量、容器尺度和场景质感；用端锅、冒泡、堆叠等可见动作修正，不凭空补品牌或价格"),
...
def audit(path: Path) -> int:
    ...
    for number in range(1, len(lines)):
        ...
        elif stripped and len(stripped) >= 12 and not stripped.startswith(("【", "```")):
            previous = seen_in_section.get(stripped)
            if previous is not None and number - previous > 1:
                findings.append((number, "疑似重复剪辑残留", stripped, "同一节出现完全相同的时间/动作/检查句；保留一次并把后文接到新的动作或情绪。若是有意复沓，确认是否改变了语气、视角或信息"))
            seen_in_section[stripped] = number
