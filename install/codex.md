...
- `mini`：只把 `dist/short-fiction-humanizer-mini.md` 作为一次性 prompt。适合上下文很紧、较弱模型或只清明显模板感；它不是完整 skill。
...
```bash
mkdir -p short-fiction-humanizer
cp SKILL.md short-fiction-humanizer/
cp -r references short-fiction-humanizer/
```
...
```markdown
## 写作风格
当任务涉及"去 AI 味""说人话""自然一点""别像模板"这类改写时，遵循 `short-fiction-humanizer/SKILL.md`。
...
```bash
codex exec -C . "读取 ./dist/short-fiction-humanizer-mini.md，按其中规则改写以下文本：..."
```
...
```bash
mkdir -p ~/.codex/skills/short-fiction-humanizer
cp SKILL.md ~/.codex/skills/short-fiction-humanizer/
cp -r references ~/.codex/skills/short-fiction-humanizer/
```
...
当任务涉及"去 AI 味""说人话""自然一点""别像模板"这类改写时，使用本地 skill `short-fiction-humanizer`。
对外文本优先按它处理；代码、日志、配置和命令输出不套这个 skill。
EOF
```
